/* 宋·澶渊之盟 加载探针（零依赖 CDP）
 * 验证：county.html?scene=song_chan_yuan 加载无 JS 异常；事件列表/地图渲染；
 *       control 层已打包（song_chan_yuan 新增 control.json）；六维覆盖 = 1..6 全。
 */
...
(async () => {
  ...
  await send('Emulation.setDeviceMetricsOverride', { width: 1400, height: 1000, deviceScaleFactor: 1,
    mobile: false, screenWidth: 1400, screenHeight: 1000 }).catch(()=>{});
  ...
  for (const sc of SCENES) {
    ...
    const ctrl = await ev(`(function(){ try {
        var s = (window.SANDBOX_SLICES && SANDBOX_SLICES['${sc}']) ||
                (window.SANDBOX_DATA && SANDBOX_DATA.scenes && SANDBOX_DATA.scenes['${sc}']) || {};
        var c = s.control || [];
        var seats = s.control_seats || [];
        return JSON.stringify({control: c.length, seats: seats.length});
    ...
    const dims = await ev(`(function(){ try {
        var s = (window.SANDBOX_SLICES && SANDBOX_SLICES['${sc}']) ||
                (window.SANDBOX_DATA && SANDBOX_DATA.scenes && SANDBOX_DATA.scenes['${sc}']) || {};
    ...
    ok((',' + dims + ',').indexOf(',1,') >= 0 && (',' + dims + ',').indexOf(',2,') >= 0 &&
        (',' + dims + ',').indexOf(',3,') >= 0 && (',' + dims + ',').indexOf(',4,') >= 0 &&
        (',' + dims + ',').indexOf(',5,') >= 0 && (',' + dims + ',').indexOf(',6,') >= 0,
        '六维全覆盖 1..6（含第5维思想、第2维技术）');
  }
