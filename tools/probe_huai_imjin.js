/* 唐淮西 + 万历朝鲜之役 加载探针（零依赖 CDP）
 * 验证：county.html?scene=X 加载无 JS 异常；事件列表/地图渲染；
 *       control 层已打包（唐淮西本次新增 control.json）；六维覆盖含第 5 维（思想）。
 */
...
(async () => {
  ...
  // 硬经验 #16：无头视口须强设宽屏，否则 @media(max-width:1080px) 触发 panel display:none
  ...
  for (const sc of SCENES) {
    ...
    // 控制层（唐淮西本次新增 control.json；imjin 原有）
    ...
    ok((',' + dims + ',').indexOf(',5,') >= 0, '六维含第 5 维（思想）');
  }
