...
def build_conflicts(assertions):
    ...
    for (subject, predicate), items in groups.items():
        ...
        spread = None
        nums = [i["value"] for i in items if isinstance(i["value"], (int, float))]
...
def main():
    ...
    sd["scenes"] = {"sarhu": sarhu, "kaiyuan": kaiyuan}

    os.makedirs(os.path.dirname(OUT), exist_ok=True)
    with open(OUT, "w", encoding="utf-8") as f:
        f.write("// 本文件由 tools/build.py 自动生成，请勿手工编辑。\n")
        f.write("// 权威数据源：data/sarhu/ 与 data/kaiyuan/\n")
    ...
    for key, sc in (("萨尔浒", sarhu), ("开原", kaiyuan)):
