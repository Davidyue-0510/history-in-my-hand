...
def _llm_extract_world(spec, conform=True):
    ...
    if spec.get("llm_fixture"):
        fx = spec["llm_fixture"]
        if not os.path.isabs(fx) and spec.get("_spec_path"):
            fx = os.path.join(os.path.dirname(os.path.abspath(spec["_spec_path"])), fx)
        with open(fx, encoding="utf-8") as f:
            raw_text = f.read()
        print("[world] 离线 llm_fixture 载入: %s" % fx)
    else:
...
def _register_scene(spec, scene_dir):
    ...
    if spec["id"] not in reg["scenes"]:
        ...
        with open(reg_path, "w", encoding="utf-8") as f:
            json.dump(reg, f, ensure_ascii=False, indent=1)
            f.write("\n")
        print("[world] 已注册场景 %s 到 scenes.json" % spec["id"])
...
def _assemble_and_register(spec, raw, scene_dir):
    """geocode + control + vocab + 写文件 + 场景级 dims + 地形/场景注册。
    单源与多源共用；不做 build/gates（由 _run_build_and_gates 收尾）。"""
    ...
    src_list = spec.get("_multi_sources") or [spec["source"]]
    parties = spec.get("_derived_parties", [src_list[0].get("party", "unknown")])
    sources_data = {"sources": [{
        "id": s.get("id", s.get("title", "")), "title": s.get("title", ""),
        "party": s.get("party", src_list[0].get("party", "unknown")),
        "stance_label": s.get("stance_label", ""), "distance_label": s.get("distance_label", ""),
    ...
    for name, data in [
        ...
        ("persons", {"persons": raw.get("persons", [])}),
        ("events", {"events": raw.get("events", [])}),
        ("places", {"places": places, "rivers": [], "wall": []}),
        ("edges", {"edges": raw.get("edges", [])}),
    ]:
    ...
    with open(os.path.join(scene_dir, "assertions.jsonl"), "w", encoding="utf-8") as f:
        for a in raw.get("assertions", []):
            f.write(json.dumps(a, ensure_ascii=False) + "\n")
    # 顺序很关键：先 _register_scene 建场景条目（recompute_scene_dims 要求条目已存在），
    # 再 recompute_scene_dims 把并集 dims 写回 scenes.json。否则单源首跑时 dims 被清空
    # 且多源会侥幸复用残留条目暴露为顺序依赖 bug（v0.79 修）。
