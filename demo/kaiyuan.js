...
(function () {
  ...
  var D = SD.scenes.kaiyuan;          // 开原县级切片
  ...
  function renderTerrainCtl() {
    ...
    (function () {
      ...
      n.innerHTML = '<i class="lay-dot"></i><span class="lay-name">进军路线</span>' +
        '<span class="lay-n">' + (on ? '开' : '关') + '</span>';
      n.title = '努尔哈赤攻开原进军路线（赫图阿拉→开原）及地形紧张度';
  ...
  function renderEvents() {
    ...
    lead.innerHTML = '开原是辽北第一重镇：明三万卫—清开原县，万历四十七年（1619）陷于后金，' +
    ...
    order.forEach(function (name) {
      ...
      var items = D.assertions.filter(function (a) {
        return state.sources.has(a.source) && PARTY_BUCKET[SRC[a.source].party] === name &&
               a.subject === 'event:kaifa';
      });
      var gaps = D.assertions.filter(function (a) {
        return a.layer === 'gap' && a.subject === 'event:kaifa' &&
               PARTY_BUCKET[SRC[a.source].party] === name;
      });
      var n = document.createElement('div');
      n.className = 'pty';
      var head = '<div class="pty-head"><span class="pty-name">' + name + '</span>' +
        '<span class="pty-n" style="background:' + b.color + '">' +
  ...
  function subjectName(s) {
    var map = {
      'army:jin_kaifa': '后金攻开原兵力', 'event:kaifa': '开铁之战',
      'place:kaiyuan_cheng': '开原城', 'person:zhengzhifan': '郑之范',
      'event:yehe': '叶赫', 'doc:kaiyuan_tushuo': '开原图说'
    };
    if (map[s]) return map[s];
    if (s.indexOf('person:') === 0) return D.persons.filter(function (p) { return p.id === s.slice(7); })[0].name;
    return s;
  }
  ...
  function selectRoute() {
    ...
    state.selection = { type: 'route', title: '进军路线 · 赫图阿拉 → 开原', sub: sub, list: list };
    goTab('inspect'); renderInspect();
  }
