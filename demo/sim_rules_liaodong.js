...
// 单一信息源 = military_seeds/liaodong.json（场景专属）+ military_seeds/_shared.json（通用 R1-R7 模型本体）。
  ...
  "params": [
    {
      ...
      "step": 0.05,
      "unit": "",
      "desc": "后金/清方每条征服成功阈值；=1 时 intrinsicP<1 恒成立→全部达成"
    },
    {
      "id": "mingExec",
      "label": "明方执行力",
    ...
    },
    {
      ...
      "step": 0.05,
      "unit": "",
      "desc": "非争议征服要求至少一邻格已属攻方才易取；<1 时无邻格需二次确定性抽检"
    },
    {
      "id": "centerStability",
      "label": "中枢稳定度(三阶层)",
      ...
      "step": 0.05,
      "unit": "",
      "desc": "center 阶层指标，阶段2仅派生展示（#13 反馈环输入）"
    },
    {
      "id": "localEduMono",
      "label": "地方教育垄断(三阶层)",
      ...
      "step": 0.05,
      "unit": "",
      "desc": "local 阶层指标；>0.7 持续3代→阶层固化（反馈环 R2）"
    },
    {
      "id": "grassrootMob",
      "label": "底层动员度(三阶层)",
      ...
      "step": 0.05,
      "unit": "",
      "desc": "grassroot 阶层指标；(1-eduMono)*贫困>0.8→革命（反馈环 R3）"
    },
    {
      "id": "factionInfightAmp",
      "label": "党派内耗强度",
      "default": 0,
      "min": 0,
      "max": 1,
      "step": 0.05,
      "unit": "",
      "desc": "启用党派因素（#3延伸）：朝堂内耗对明方执行力的折扣系数；=0 关闭→100%重放，>0 时各派内斗削弱前线(明方攻防/守土)"
    },
    {
      "id": "logisticsPenalty",
      "label": "后勤物理约束",
      ...
      "step": 0.05,
      "unit": "",
      "desc": "启用 whatif 物理极限：超补给半径/冬季行军对攻击方执行力的折扣系数；=0 关闭→重放"
    },
    {
      "id": "winterPenalty",
      "label": "冬季行军系数",
      ...
      "step": 0.05,
      "unit": "",
      "desc": "冬季有效行军速×该系数（whatif 同源：雪阻路冻）；仅 logisticsPenalty>0 时生效"
    }
  ],
  "rules": [
    {
    ...
    },
    {
      "id": "R6",
      "type": "feedback_court",
      "desc": "党派内耗→明方削弱（#3延伸）：每年活跃派系(period_years 覆盖当年)越多、利益越分化、朝堂凝聚越低、越确信「明不亡」而越不愿增援，则内耗越高。① 对明方作场的转移，明方执行力 = mingExec×(1−infightAmp×内耗)；② 防御侧(明方为 from)攻方更易得手 eff×min(1,1+infightAmp×内耗×0.6)；③ 模型推演：内耗高时部分明方治所守军溃散降为 contested（确定性哈希命中，非史载转移）。全部确定性（无 RNG）。派系参数（power/selfInterest/reinforce/beliefNoFall）单一真值 = data/vocab/ming_qing.json，经 tools/gen_faction_bundle.py 编译为 demo/_faction_ming.js，禁止在 demo 内硬编码。",
    ...
    },
    {
      ...
      "desc": "whatif 物理极限→攻击方投送折扣（后勤/行军模型同源）：攻击方需自最近已控治所投送兵力。v0.63 提升为「必开」物理维度——三约束始终计算并展示：①超补给半径(明300km/清400km)：投送不可达，战役无法维持，执行力压至近似0（硬失败）；②半径内：距最近治所越远投送越难，叠加冬季雪阻路冻(winterPenalty)与基础辎重摩擦，按比例削减攻击方执行力（软失败）；③抵达时间窗（估算行军天数 vs 季节可行窗口）、④扎营水源(BM.rivers 邻近判定)、⑤粮草可持续(假定兵力×日耗×行军天数 vs 30日上限)。季节按 seedHash 确定性派生，少量关键战役用史载月份。折扣仅在 logisticsPenalty>0 时施加（=0→忠实重放）。",
      "source": "whatif.html 后勤/行军模型（RATES/RATION/WINTER_RATE/补给半径）+ v0.63 三约束扩展",
      "formula": "if dist>radius: eff=min(eff,0.015); else: gentle=clamp(1-(dist/radius)*0.35,0.45,1); baseFric=1-logisticsPenalty*0.4; factor=gentle*winterFactor*baseFric*(1-timePenalty)*(1-waterPenalty)*(1-foodPenalty); eff*=factor",
      "activeInPhase": "2"
    }
  ],
  "branches": [
  ...
  "missing_dims": {
    "probability": {
      ...
      "dist": {
        ...
        "interpretation": "intrinsicP < eff(to) 则成功"
      }
    },
    "logistics": {
      "dim": 9,
      "status": "implemented-required",
      "desc": "物理维度提升为必开（v0.63）：R7 已整合进 sim_engine 物理层，每处征服按最近已控基地投送距离+季节判定（超补给半径/冬季按比例削减攻击方执行力），并新增三约束——①抵达时间窗 ②扎营水源(BM.rivers 邻近判定) ③粮草可持续(假定兵力×日耗×行军天数 vs 30日上限)。同源 whatif.html 行军/粮草模型；折扣仅在 logisticsPenalty>0 时施加（=0→忠实重放），故该维度「必开」指其始终被计算与展示，而非默认即打折。",
      "dist": {
        "type": "prior",
        "mean": 0.6,
        "sd": 0.2,
